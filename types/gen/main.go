...
type testDef struct {
	Package string // pod
	Type    string // Pod
	FQType  string // v1.Pod
}

func main() {

	if len(os.Args) != 2 {
		fmt.Fprintf(os.Stdout, "USAGE: %v: <type-name>\n", os.Args[0])
		os.Exit(1)
	}

	def := testDef{FQType: os.Args[1]}

	parts := strings.Split(def.FQType, ".")

	if len(parts) != 2 {
		fmt.Fprintln(os.Stdout, "invalid type name (example: v1.Pod)")
		os.Exit(1)
	}

	def.Type = parts[1]
	def.Package = strings.ToLower(parts[1])

	buf := processTemplate(&def)
	buf = processImports(&def, buf)
...
func processTemplate(def *testDef) []byte {
	buf := &bytes.Buffer{}
	if err := testTemplate.Execute(buf, &def); err != nil {
		panic(err)
	}
	return buf.Bytes()
}

func processImports(def *testDef, in []byte) []byte {
	file := path.Join("./types", def.Package, "generated_test.go")
	out, err := imports.Process(file, in, nil)
	if err != nil {
		panic(err)
	}
	return out
...
func testGenObject(ns, name, vsn string) *{{.FQType}} {
	return &{{.FQType}}{
